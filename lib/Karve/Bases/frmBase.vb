...
#Region "Variables"
    ...
    Protected pkEdit As New PrimaryKey
...
#Region "Propiedades Desplazamiento"
...
#End Region
    ...
    <Category(CatKarve)> _
    Public Property CodigoEdicion As String
        Get
            Return pkEdit.CodEdit
        End Get
        Set(value As String)
            pkEdit.CodEdit = value
        End Set
    End Property
    ...
    Private Sub prepareSql()
        ...
        If Not IsNothing(tablasEdit) Then
            ...
            For Each table As TablaEdit In tablasEdit
                mySqlEdit &= "select * from " & table.Tabla & " where "
                Dim i As Integer = table.CamposID.Count
                For Each pk As PrimaryKey In table.CamposID

                    If pk.Expresion <> "" Then
                        mySqlEdit &= pk.CampoID & " = " & pk.Expresion.Replace("%codEdit%", pk.CodEdit)
                    Else
        ...
        Try
            ...
            If Not datosIncorrectos Then
                ...
                If newReg Then
                    ...
                    For Each table As TablaEdit In tablasEdit
                        For Each pk As PrimaryKey In table.CamposID
                            If pk.CodEdit <> "" Then dtsEdit.Tables(table.TableAlias).Rows(0).Item(pk.CampoID) = pk.CodEdit
                        Next
                    Next
                    If errorRF Then
                        kMsgBox.Print("La informacion de los siguientes campos no se ha vinculado a la base de datos y no será actualizada.", _
                ...
                newReg = False
                loadAfterSave()
                RaiseEvent datosGuardados(pkEdit.CodEdit, New EventArgs)
...
#End Region 'FUNCIONES PUBLICAS PARA LAS FUNCIONALIDADES INTERNAS
...
#Region "Bloqueo de Registro"
    ...
    Private Sub checkBloqueoRegistro()
        mySqlBlock = "select NOMBRE, INICIO from CONTROL_BLOQUEO_REGISTROS as C inner join USURE as U on C.USUARIO = U.CODIGO " & _
            "where C.TABLA = '" & tablasEdit(0).Tabla & "' and C.REGISTRO = '" & pkEdit.CodEdit & "'"
    ...
    Private Sub strBloqueo()
        Dim insert As String = "insert into CONTROL_BLOQUEO_REGISTROS " & _
                                ...
                                "('" & tablasEdit(0).Tabla & "', '" & pkEdit.CodEdit & "', '" & US.GStr("CODIGO") & "', getdate(), getdate())"
    ...
    Private Sub updBloqueo()
        Dim update As String = "update CONTROL_BLOQUEO_REGISTROS " & _
                                ...
                                "where tabla = '" & tablasEdit(0).Tabla & "' and registro = '" & pkEdit.CodEdit & "'"
...
#Region "   .   Función Desplazamiento.  "
    ...
    Public Sub LoadDesplazamiento()
        ...
        btnPrimero.Enabled = _iDesplaza - 1 > 0
        btnAnterior.Enabled = btnPrimero.Enabled
        btnUltimo.Enabled = _iDesplaza < _Desplazamiento.RegistroFinal()
        btnSiguiente.Enabled = btnUltimo.Enabled
