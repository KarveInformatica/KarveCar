...
namespace Users.ViewModels
{
    internal class UserControlViewModel : KarveControlViewModel, ICreateRegionManagerScope, INavigationAware
    {
		...
		private INotifyTaskCompletion<IEnumerable<UserSummaryDto>> _userCompletion;
		...
		public UserControlViewModel(IDataServices services,
		   ...
		   {
		     _userService = services.GetUserDataService();
			 _regionManager = manager;
		     ItemName = Usuarios;
		     DefaultPageSize = 50;
             ViewModelUri = new Uri("accounting/users/viewmodel?id=" + UniqueId);
             InitViewModel();
		   }
        ...
        private void InitViewModel()
        {
            ...
            SubSystem = DataSubSystem.Account;
            EventManager.RegisterObserverSubsystem(EventSubsystem.AccountVm, this);
		...
		public override void StartAndNotify()
        {
            ...
            _reservationCompletion = NotifyTaskCompletion.Create<IEnumerable<UserSummary>>(
               _reservationRequestDataService.GetPagedSummaryDoAsync(1, DefaultPageSize), (task, ev) =>
               {
                   if (task is INotifyTaskCompletion<IEnumerable<UserSummary>> summaryCollection)
                   {
                       if (summaryCollection.IsSuccessfullyCompleted)
                       {
                           ...
                           {
                               var maxItems = _userService.NumberItems;
                               PageCount = __userService.NumberPage;
        ...
        protected override void NewItem()
        {
            ...
            currentPayload.Subsystem = DataSubSystem.Account;
            currentPayload.PrimaryKeyValue = id;
            ActiveSubSystem();
            currentPayload.Sender = ViewModelUri.ToString();
            EventManager.NotifyObserverSubsystem(Karve.Constants.GroupUser, currentPayload);
        ...
        {
            var route = @"karve://accounting/users/" + name;
        ...
        public override void DisposeEvents()
        {
            ...
            EventManager.DeleteObserverSubSystem(EventSubsystem.AccountVm, this);
