...
namespace MasterModule.ViewModels
{
    ...
    class ProviderInfoViewModel : BindableBase, IEventObserver
    {
            ...
            IDataServices dataServices)
        {
            ...
            _eventManager.registerObserverSubsystem(MasterModule.NAME, this);

        }
        /// <summary>
        ///  This changes the rows for the contacts.
        /// </summary>
        ...
        private void SetItemSourceTable(DataSet set, ref ObservableCollection<IUiObject> uiDfObjects)
        {
            ...
            foreach (IUiObject uiObject in uiDfObjects)
            {
                ...
                //  outCollection.Add(uiObject);


            }
            // uiDfObjects = outCollection;
        }
        ...
        private void LoadUserInterfaceObjects()
        {
            ...
            IDictionary<string, ObservableCollection<IUiObject>> pageObjects = generalPageBuilder.BuildPageObjects(AssistQueryRequestHandler, OnChangedField);
            _upperPartObservableCollection = pageObjects[ProviderConstants.UiUpperPart];
            _middlePartObservableCollection = pageObjects[ProviderConstants.UiMiddlePartPage];
            pageObjects = headerPageBuilder.BuildPageObjects(AssistQueryRequestHandler, OnChangedField);
            _headerObservableCollection = pageObjects[ProviderConstants.UiUpperPart];
            ...
            _componentsList.Add(_upperPartObservableCollection);
            _componentsList.Add(_middlePartObservableCollection);
        ...
        private void Init(string primaryKeyValue, bool insert, DataSet currentDataSet = null, DataSet helper = null)
        {
            ...
            {
                SetItemSourceTable(currentDataSet, ref _upperPartObservableCollection);
                SetItemSourceTable(currentDataSet, ref _middlePartObservableCollection);
            //   MessageBox.Show("Name");
                 SetSourceViewTable(helper, ref _upperPartObservableCollection);
                    SetSourceViewTable(helper, ref _middlePartObservableCollection);
            }
            IsVisible = Visibility.Visible;
            _initializationTable = NotifyTaskCompletion.Create<IList<DataSet>>(LoadDataValue(primaryKeyValue, insert));
        ...
        private async Task<IList<DataSet>> LoadDataValue(string primaryKeyValue, bool insert)
        {
            ...
            SetSourceViewTable(assistDataSet, ref _upperPartObservableCollection);
            SetSourceViewTable(assistDataSet, ref _middlePartObservableCollection);
        ...
        public void incomingPayload(DataPayLoad dataPayLoad)
        {
            ...
            if (_primaryKeyValue == payload.PrimaryKeyValue)
            {
                ...
                {
                    IList<DataSet> currentDataSet = null;
                    if (payload.HasDataSetList)
                    {
                        currentDataSet = payload.SetList;
                    }
                    // TODO: check values.
                    if (currentDataSet != null)
                    {
                        Init(_primaryKeyValue, false, currentDataSet[0], currentDataSet[1]);
                    }
