...
namespace DataAccessLayer.Model
{
        ...
        public void InitializeMapping()
        {
            var mapperConfiguration = new MapperConfiguration(cfg =>
            {
                ...
                {       var contactDto = new ContactsDto();
                        ...
                        contactDto.ContactId = Int32.Parse(src.ccoIdContacto);
    ...
    internal class PocoToProvee1 : ITypeConverter<SupplierPoco, PROVEE1>
    {
        ...
        public async Task<bool> SaveBranches(IDbConnection connection)
        {
            ...
            {
                ProDelega delega = new ProDelega();
                delega.cldDelegacion = branch.Branch;
                delega.cldIdDelega = branch.BranchId.ToString();
                if (branch.BranchId == 0)
                {
                    Random rand = new Random();
                    delega.cldIdDelega = rand.Next(1, 20000).ToString();
                }
                delega.cldDireccion1 = branch.Address;
                delega.cldDireccion2 = branch.Address2;
                delega.cldEMail = branch.Email;
                delega.cldIdCliente = this.Value.NUM_PROVEE;
                delega.cldPoblacion = branch.City;
                delega.cldTelefono1 = branch.Phone;
                delega.cldTelefono2 = branch.Phone2;
                try

                {
                    await connection.InsertAsync(delega);
                }
                catch (System.Exception e)
                {
                    retValue = false;
        ...
        public async Task<bool> SaveContacts(IDbConnection connection)
        {
            ...
            foreach (var c in this.ContactsDtos)
            {
                ...
                {
                    Random rand = new Random();
                    cont.ccoIdContacto = rand.Next(1, 20000).ToString();
                }
                try
                ...
                else
                {
                    try
                    {
                        retValue = await connection.UpdateAsync<ProContactos>(cont);
                    }
                    catch (System.Exception e)
                    {
                        string message = "Transaction Scope Exception in Vehicle Update. Reason: " + e.Message;
                        logger.Error(message);
                        DataLayerExecutionException dataLayer = new DataLayerExecutionException(message);
                        throw dataLayer;
                    }
        ...
        public async Task<bool> LoadValue(IDictionary<string, string> fields, string code)
        {
            ...
            using (IDbConnection connection = _sqlExecutor.OpenNewDbConnection())
            {
                try
                {
                    ...
                    // poblacion mapping
                    var cityMapper = MapperField.GetMapper();
                    var cityQuery = string.Format(_queryCity, _supplierValue.CP);
