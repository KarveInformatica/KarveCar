...
namespace KarveGrid.GridDefinicion
{
    public class DataGridDefinicion
    {
        ...
        {
            string _sSQL = " SELECT LIST(DISTINCT ISNULL(" + VD.setApostrofa(sAlias) + " + COLUMN_NAME, ''), ', ' ORDER BY COLUMN_ID) AS PK " + Constants.vbNewLine + " FROM SYSCOLUMN SC " + Constants.vbNewLine + " INNER JOIN SYSTABLE ST ON ST.TABLE_ID = SC.TABLE_ID " + Constants.vbNewLine + " WHERE TABLE_NAME = " + VD.setApostrofa(sTabla) + Constants.vbNewLine + " AND PKEY = 'Y' ";
        ...
        private string BeginSQL(DataGridTables DGTS, string sSQL, string sCols)
        {
            ...
            for (iRg = 0; iRg <= DGTa.Count - 1; iRg++) {
                ...
                if (DGT.TablasVirtual.Count != 0) {
                    ...
                    sBegin = sBegin + "    (   " + Constants.vbNewLine + BeginSQL(DGT.TablasVirtual, sBegin, sColsX) + Constants.vbNewLine + "    ) " + DGT.AliasTabla + " " + DGT.Criterio;
